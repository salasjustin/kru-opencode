...
`grep -rn "<name>"` across `agents/`, `skill/kru-lead/`, and the other agent files. Every hit that isn't the seat's own definition is an **inbound reference** — a sibling that delegates to it, a routing row, a "see `<name>`" boundary clause in another seat's description. A dangling reference is the drift retire exists to prevent. List them; each must be rewritten or removed in step 2.

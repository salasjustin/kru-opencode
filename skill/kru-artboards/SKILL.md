---
name: kru-artboards
description: Draft a look as local artboards — self-contained .dc.html files rendering 2-4 directions or a screen set, written into the repo's design-work path for the user to open in a browser. The fork's replacement for the bundled canvas surface; use when a look is unsettled and someone needs to see it before a screen is built.
---

The artboard mechanics `ui-designer` draws on, and the fork's whole design surface: upstream settles a look in a canvas editor the user reshapes directly, and opencode reaches no such surface. What it reaches is a browser and a file. So a direction is a **static render the user opens**, and the reshape loop is a re-brief rather than a drag (`{KRU_HOME}/kru/references/ui-practice.md` → *The artboards*).
...
- **One viewport per file, stated.** Draw at 1280 for desktop and 375 for mobile; if a direction only makes sense at both, that is two files, labelled. Do not build a responsive artboard — the breakpoint behaviour is the builder's and a half-working one reads as a defect in the look.
...
- **Web-safe faces, or a `<link>` to Google Fonts.** Nothing else loads — no CDN scripts, no images the repo doesn't have. Where art is wanted, route it to `kru/graphic-designer` before the draft and place the file it returns.

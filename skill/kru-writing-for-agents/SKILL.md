---
name: kru-writing-for-agents
description: Writing documents for agents. Use when creating or editing skills or agent definitions (agent/kru/*.md), when modifying AGENTS.md, or when deciding whether a repo fact you just discovered earns a line in one.
---

<!-- vendored from `mattpocock/skills` (v1.2.0, sha 2ffb184) → `skills/writing-for-agents`. The team's agent-document authoring standard (was `writing-great-skills` before v1.2.0). Body verbatim minus the Codex `agents/openai.yaml` metadata (unused here). Three deviations, all re-applied after any re-sync — (1) and (2) in the `description`, (3) one line in the body: (1) this repo's `agent/kru/*.md` branch, which upstream's omits — `SOURCES.md` and `ROSTER.md` both send seat authoring here, so the pointer has to fire on it; (2) the discovery branch — whether a repo fact just learned earns a line in an always-loaded file. Without (2) the pointer only fires once the decision to write is already made, so a seat that learns something durable mid-build never reaches the cache rule under *Pruning* that answers it; (3) the pointer to `REFUSALS.md` after the *Negation* paragraph — repo-owned material upstream has no branch for, disclosed behind one line so a re-sync costs only that line. Re-download from https://raw.githubusercontent.com/mattpocock/skills/main/skills/productivity/writing-for-agents/. -->

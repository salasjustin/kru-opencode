...
**A repo owns what it is.** No stamp (Step 0), and the work is more than a one-off → name **`/kru/setup`** once: it maps the repo and writes the answers — the `general` pass over stack, framework versions, architecture, conventions and test setup. Either way you reach Step 3 with the stack settled and the conventions named, and you match those over your own defaults — minimal diff, in-style.
...
Then judge **triviality**: a typo/rename/small mechanical fix goes straight to routing; anything non-trivial is a grilling candidate (Step 2.5). And map **the files this change touches** with `general` where routing alone doesn't locate them — a repo's answers map the repo, never the diff.
...
- **Too big/foggy to slice up front** → `planner` (wayfinder mode) charts a map + initial **decision tickets** (questions whose resolution is a decision, not build slices); work it one ticket per session, except the research tickets, which the charting session fires off to `general` subagents in parallel.
